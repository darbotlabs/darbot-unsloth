...
def sft_prepare_dataset(
    ...
    if do_tokenize:
        if do_formatting_func:
            ...
            if not isinstance(test_text, str):
                raise ValueError(
                    "Unsloth: TRL 1.x `formatting_func` must return a string for each example."
                )
        ...
        def _tokenize(example):
            return tokenizer(
                example[dataset_text_field] if not do_formatting_func else [
                    formatting_func(dict(zip(example, row)))
                    for row in zip(*(example[key] for key in example))
                ],
    ...
    if packing:
        ...
        # Keep TRL's precomputed loss masks and labels aligned with packed IDs.
        for column in ("labels", "completion_mask", "assistant_masks"):
            if column in dataset.column_names and column not in used_column_names:
                used_column_names.append(column)
        if isinstance(dataset, Dataset):
            from unsloth_zoo.dataset_num_proc import get_dataset_num_proc
            map_kwargs.update(num_proc = get_dataset_num_proc(
                getattr(args, "dataset_num_proc", None),
            ))
