...
def unsloth_train(trainer):
    ...
    if data_collator is None:
        ...
        if processing_class is None:
            raise ValueError("Unsloth: A processing_class is required when no data collator is supplied.")
    ...
    decay_parameters = frozenset(trainer.get_decay_parameter_names(model))
    yes_decay, no_decay = [], []
    ...
    lr_scheduler = transformers_get_scheduler(
        ...
        scheduler_specific_kwargs = getattr(training_args, "lr_scheduler_kwargs", {}),
    )
