...
def _fall_back_to_eager_on_recompile_limit(compiled_func, eager_func, label):
    ...
    def wrapper(*args, **kwargs):
        # Torch 2.14 fullgraph wrappers raise when Dynamo is explicitly disabled:
        # zero compiled frames is now an error, not an implicit eager execution.
        if state["eager"] or getattr(torch._dynamo.config, "disable", False):
            return eager_func(*args, **kwargs)
